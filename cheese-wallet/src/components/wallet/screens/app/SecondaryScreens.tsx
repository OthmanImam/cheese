  ...
  return (
    <div className="screen active" id="screen-applock">
      <div className="applock-wrap">
        ...
        <div className="applock-sub">{user ? `Hello, ${user.fullName}` : 'Enter your 4-digit PIN'}</div>
        <PinPad
          value={pin}
          onChange={setPin}
...
export function KycScreen() {
  ...
  const tier: 'silver' | 'gold' | 'black' = 'silver'
          ...
          <>
            ...
            <div className="kyc-tier-cards">
              {[
                ...
                { name:'GOLD',   badge:'Upgrade', badgeCls:'gold',   limit:'$5,000 / day · $25,000 / month', benefits:'All Silver · Higher limits · Earn yield · Priority support', isCurrent: false, onClick: startFlow },
...
export function ProfileEditScreen() {
  ...
  const [name,     setName]    = useState(displayUser ? `${displayUser.fullName}` : '')
  const [username, setUsername]= useState(displayUser?.username  ?? '')
  const [phone,    setPhone]   = useState(displayUser?.phone     ?? '')

  const initials = (name || (displayUser ? `${displayUser.fullName}` : 'OA')).split(' ').map((w: string) => w[0]).join('').slice(0,2).toUpperCase()
