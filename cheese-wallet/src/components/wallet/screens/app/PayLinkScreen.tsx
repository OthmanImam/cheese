...
const STATUS_BADGE: Record<string, { label: string; color: string; bg: string }> = {
  active:    { label: 'Active',    color: '#C9A84C', bg: 'rgba(201,168,76,0.12)'  },
...
function PaymentLinkCard({
  ...
  const shareText = `${user?.fullName ?? 'Someone'} is requesting ${fmtNgn(amount, ngnRate)} (${fmtUsdc(amount)} USDC). Pay here: ${result.link}`
  ...
  const shareTelegram = () => {
    const url = `https://t.me/share/url?url=${encodeURIComponent(result.link)}&text=${encodeURIComponent(shareText)}`
    window.open(url, '_blank', 'noopener')
  }
  ...
  const nativeShare = async () => {
    ...
    try {
      await navigator.share({ title: 'Payment Request', text: shareText, url: result.link })
    } catch {
      // user cancelled
    }
  }
...
function MyRequestsPanel() {
  ...
  const links: (CreatePayLinkResponse & { status: string; paidAt?: string })[] = data?.links ?? []
