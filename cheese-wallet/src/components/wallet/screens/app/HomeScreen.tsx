...
export function HomeScreen() {
  ...
  const usd  = parseFloat(balance?.totalUSDC ?? '0')
  const rateVal = parseFloat(rate?.effectiveRate ?? '1610')
  const ngn  = usd * rateVal
  ...
  function handleTxTap(tx: Transaction) {
    ...
    const canRepeat = !isIn && tx.type !== 'card_payment'
    const method = (tx.type === 'bank_transfer' ? 'account'
                    : tx.recipientAddress     ? 'evm'
                    : 'username') as 'account' | 'evm' | 'username'
    const recipient = method === 'account' ? tx.accountNumber : method === 'evm' ? tx.recipientAddress : tx.recipientUsername
    openTxDetail({
      type:    isIn ? 'in' : tx.type === 'card_payment' ? 'card_spend' : 'out',
      amount:  (isIn ? '+' : '-') + '$' + Number(tx.amountUsdc ?? tx.amountNgn ?? 0).toFixed(2),
      desc:    tx.description ?? tx.recipientUsername ?? tx.recipientAddress ?? 'Transaction',
      ...
      fee:     tx.feeUsdc ? `$${Number(tx.feeUsdc).toFixed(2)}` : '$0.00 — Free',
      status:  tx.status as 'confirmed' | 'pending' | 'failed',
      ...(canRepeat && recipient ? {
        repeatPayload: {
          method,
          recipient:     recipient,
          recipientName: tx.recipientName ?? recipient ?? '',
  ...
  return (
    <div className="screen active" id="screen-home">
      ...
      <div className="home-hero">
        <div className="hero-card">
          <div className="hero-card-inner">
            ...
            <div className="hero-name">{user ? `${user.fullName}` : 'Loading…'}</div>
      ...
      <div className="quick-actions">
        ...
        <div className="qa-btn" onClick={() => goTo('paylink')}>
          <div className="qa-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></div>
          <span>Request</span>
        </div>
      </div>
...
function TxRow({ tx, onClick }: { tx: Transaction; onClick: () => void }) {
  ...
  const initial= ((tx.description ?? tx.recipientUsername ?? tx.recipientAddress ?? tx.recipientName ?? '?').split(' ').pop() ?? '?')[0].toUpperCase()
  ...
  return (
    <div className="tx-item" onClick={onClick} style={{ cursor:'pointer' }}>
      ...
      <div className="tx-info">
        <div className="tx-name">{tx.description ?? tx.recipientUsername ?? tx.recipientAddress ?? tx.recipientName ?? 'Transaction'}</div>
