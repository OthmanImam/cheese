...
type FilterType = 'all' | 'deposit' | 'withdrawal' | 'send_username' | 'send_address' | 'bank_transfer' | 'card_payment'
const FILTER_LABELS: { key: FilterType; label: string }[] = [
  { key: 'all',       label: 'All'      },
  { key: 'deposit',   label: 'Deposits' },
  { key: 'withdrawal',label: 'Withdrawals' },
  { key: 'send_username', label: 'Sent to Username' },
  { key: 'send_address', label: 'Sent to Address' },
  { key: 'bank_transfer', label: 'Bank Transfers' },
  { key: 'card_payment', label: 'Card Payments' },
]
...
export function HistoryScreen() {
  ...
  useEffect(() => {
    ...
    if (page === 1) {
      setAccumulated(data.transactions)
    } else {
      setAccumulated(prev => {
...
function HistoryRow({ tx, onClick }: { tx: Transaction; onClick: () => void }) {
  ...
  const label  = tx.description ?? tx.recipientUsername ?? tx.recipientAddress ?? tx.recipientName ?? 'Transaction'
