  ...
  async function handleLogin() {
    ...
    try {
      await login.mutateAsync({
        identifier,
        password,
        deviceSignature: 'mock-sig',   // replaced at integration
...
export function DeviceScreen() {
  ...
  async function generate() {
    setGenerating(true)
    // Real implementation: use @noble/ed25519 or WebCrypto
    // Prototype: generate mock key pair
    await new Promise(r => setTimeout(r, 1200))
    const mockId  = 'CHZ-' + Math.random().toString(36).slice(2,8).toUpperCase()
    const mockPub = Array.from({ length: 64 }, () => Math.floor(Math.random()*16).toString(16)).join('')
    setDeviceId(mockId)
    setPubKey(mockPub)
    setGenerated(true)
    setGenerating(false)
