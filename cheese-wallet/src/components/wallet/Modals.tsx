...
export function AskReceiveModal() {
  ...
  function copyUsername() {
    navigator.clipboard?.writeText('cheesewallet.app/@seun.adeyemi').catch(() => {})
    showToast('Copied', 'Payment link copied to clipboard')
  }
  function shareLink() {
    if (navigator.share) {
      navigator.share({ title: 'Send me USDC', url: 'https://cheesewallet.app/@seun.adeyemi' })
    } else {
      copyUsername()
    }
  }
    ...
    <ModalOverlay id="askReceive">
      ...
      <div style={{ padding: '0 24px 24px' }}>
        ...
        <div style={{ textAlign: 'center', padding: '16px 0 20px' }}>
          ...
          <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, color: 'var(--text)', fontWeight: 700 }}>@seun.adeyemi</div>
          <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>Anyone can send to this username — no fees</div>
        </div>

        {/* QR placeholder */}
        <div style={{ background: 'var(--bg3)', border: '1px solid rgba(201,168,76,0.12)', borderRadius: 16, padding: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16, minHeight: 140 }}>
          <div style={{ textAlign: 'center', opacity: 0.5 }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>📱</div>
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>QR code appears here</div>
          </div>
        </div>
...
export function BankFlowModal() {
  ...
  async function handlePin(entered: string) {
    ...
    // TODO: real PIN verification via API
    if (entered === '1234') {
      ...
      try {
        ...
        showToast('Transfer failed', e?.message ?? 'Please try again')
        setPinErr(true)
        setTimeout(() => { setPinErr(false); setPin('') }, 800)
      }
    } else {
      setPinErr(true)
      setTimeout(() => { setPinErr(false); setPin('') }, 700)
    }
  }
    ...
    <div className="modal-overlay bank-flow-modal show" onClick={e => e.target === e.currentTarget && close()}>
      <div className="modal-sheet" onClick={e => e.stopPropagation()}>
        ...
        <div className="bflow-body">
          ...
          {step === 'pin' && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              {pinErr && <ErrorBanner message="Incorrect PIN. Please try again." />}
              <PinPad
                value={pin}
                onChange={setPin}
