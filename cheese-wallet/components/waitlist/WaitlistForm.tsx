...
import { submitWaitlist, UsernameStatus } from "@/lib/waitlist";
...
export default function WaitlistForm({ onSuccess }: WaitlistFormProps) {
  ...
  const handleUsernameStatus = useCallback((s: UsernameStatus) => {
    setUsernameStatus(s);
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    setServerError(null);
    try {
      const result = await submitWaitlist({ email, username });
      if (result.success) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      ...
      <div>
        ...
        <input
          ...
          onBlur={(e) => {
            ...
            e.currentTarget.style.borderColor = emailBorder;
          }}
        />
        {emailError && (
      ...
      <button
        ...
        style={{
          width: "100%",
          background: formReady ? "var(--gold)" : "rgba(201,168,76,0.2)",
          color: formReady ? "var(--black)" : "rgba(245,238,216,0.3)",
          border: "none",
          padding: "18px 24px",
        ...
        onMouseEnter={(e) => { if (formReady) (e.currentTarget as HTMLElement).style.background = "var(--gold-light)"; }}
        onMouseLeave={(e) => { if (formReady) (e.currentTarget as HTMLElement).style.background = "var(--gold)"; }}
      >
        {submitting ? (
          <>
