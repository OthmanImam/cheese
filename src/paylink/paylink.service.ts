...
import { PaymentRequest, PayLinkStatus } from './entities/payment-request.entity'
...
export class PayLinkService {
  ...
  private buildUrl(username: string, amountUsdc: string, token: string): string {
    const base    = this.config.get<string>('app.payLinkBaseUrl', 'https://cheesepay.xyz')
  ...
  async createPayLink(creatorId: string, dto: CreatePayLinkDto) {
    ...
    if (isNaN(amount) || amount < MIN_USDC)
      throw new BadRequestException(`Minimum request amount is $${MIN_USDC} USDC`)
    if (amount > MAX_USDC)
      throw new BadRequestException(`Maximum request amount is $${MAX_USDC.toLocaleString()} USDC`)

    const creator = await this.userRepo.findOne({ where: { id: creatorId, isActive: true } })
    if (!creator) throw new NotFoundException('User not found')
    if (!creator.stellarPublicKey)
      throw new BadRequestException('Your wallet is not fully set up yet')

    const expiryHours = Math.min(dto.expiresInHours ?? DEFAULT_EXPIRY_HOURS, MAX_EXPIRY_HOURS)
    ...
    this.logger.log(`PayLink created: @${creator.username} $${amount.toFixed(2)} USDC — ${token}`)

    return { id: pr.id, url, token, amountUsdc: pr.amountUsdc, note: pr.note, expiresAt, expiresInHours: expiryHours }
  }

  // ── GET /paylink/:token (public) ──────────────────────────
  async resolveLink(token: string): Promise<PayLinkView> {
    const pr = await this.prRepo.findOne({ where: { token }, relations: ['creator', 'payer'] })
  ...
  async payLink(payerId: string, token: string, dto: PayLinkPayDto, payerIp?: string) {
    // 1. Load request
    const pr = await this.prRepo.findOne({ where: { token }, relations: ['creator'] })
    if (!pr) throw new NotFoundException('Payment link not found')
    ...
    if (pr.status === PayLinkStatus.PAID)
      throw new ConflictException('Payment already completed — double payment prevented')
    ...
    // 2. Load payer
    const payer = await this.userRepo.findOne({ where: { id: payerId, isActive: true } })
    ...
    if (!device) throw new ForbiddenException('Device not recognized. Please re-register your device.')
    ...
    // 7. Balance check
    const { usdc: rawBalance } = await this.stellarService.getUsdcBalance(payer.stellarPublicKey)
    ...
    try {
      ...
      if (locked.expiresAt < new Date()) {
        await queryRunner.manager.update(PaymentRequest, pr.id, { status: PayLinkStatus.EXPIRED })
    ...
    try {
      ...
      this.logger.error(`PayLink Stellar send failed: ${err.message}`, err.stack)
      // Revert DB gate so payer can retry
      await this.prRepo.update(pr.id, { status: PayLinkStatus.PENDING, payerId: null, paidAt: null, payerIp: null })
      throw new BadRequestException('Network transfer failed. No funds were moved. Please try again.')
    }
    ...
    const tx = await this.txService.create({
      ...
      txHash,
      reference:         ref,
      description:       pr.note ? `PayLink: "${pr.note}"` : `PayLink → @${pr.creator.username}`,
    })

    // 11. Persist on-chain receipt
    await this.prRepo.update(pr.id, { settledTxId: tx.id, settledTxHash: txHash })

    const paidAt = new Date()
    const frontendUrl = this.config.get<string>('app.frontendUrl', 'https://cheesewallet.app')
    ...
    this.emailService.sendMoneyReceived({
      to:         pr.creator.email,
    ...
    this.emailService.sendMoneySent({
      to:                payer.email,
    ...
    return { txId: tx.id, txHash, amountUsdc: amount.toFixed(6), fee: fee.toFixed(6), paidAt }
  }
  ...
  async myLinks(creatorId: string, page = 1, pageSize = 20) {
    ...
    const now      = new Date()
    const stale    = rows.filter(r => r.status === PayLinkStatus.PENDING && r.expiresAt < now)
    if (stale.length) {
      await this.prRepo.update(stale.map(r => r.id), { status: PayLinkStatus.EXPIRED })
      stale.forEach(r => { r.status = PayLinkStatus.EXPIRED })
    }
  ...
  async cancelLink(creatorId: string, token: string) {
    ...
    if (pr.creatorId !== creatorId)
      throw new ForbiddenException('You can only cancel your own payment requests')
    if (pr.status === PayLinkStatus.PAID)
      throw new ConflictException('Cannot cancel a link that has already been paid')
    if (pr.status === PayLinkStatus.CANCELLED)
  ...
  async expireStaleLinks(): Promise<void> {
    ...
    if ((result.affected ?? 0) > 0)
      this.logger.log(`Cron: expired ${result.affected} stale payment request(s)`)
  }
}
