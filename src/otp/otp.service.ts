...
export class OtpService {
  ...
  async sendOtp(email: string, type: OtpType, meta?: { fullName?: string }): Promise<void> {
    ...
    if ([OtpType.EMAIL_VERIFY, OtpType.PHONE_VERIFY, OtpType.LOGIN_2FA].includes(type)) {
      await this.emailService.sendSignupOtp({ to: email, fullName, otp: code, expiresIn })
    } else if (type === OtpType.PASSWORD_RESET) {
      await this.emailService.sendPasswordResetOtp({ to: email, fullName, otp: code, expiresIn })
    }
  }
  ...
  async verifyOtp(email: string, code: string, type: OtpType): Promise<boolean> {
    ...
    if (otp.attempts >= 5)          throw new BadRequestException('Too many failed attempts. Request a new OTP.')
  ...
  async checkOtp(email: string, code: string, type: OtpType): Promise<Otp> {
    ...
    if (!otp || otp.expiresAt < new Date()) throw new BadRequestException('Invalid or expired OTP')
